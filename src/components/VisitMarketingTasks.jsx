 ...
 */
export default function VisitMarketingTasks({ entityType, entityId, marketId, reloadTrigger }) {
  ...
  // Compute only marketing tasks and sort newest -> oldest by visit_date
  const items = useMemo(() => {
    const only = (Array.isArray(visits) ? visits : [])
      .filter(v => {
        const txt = (v?.marketing_tasks ?? '').trim();
  ...
  useEffect(() => {
    let cancelled = false;
    const ids = Array.from(new Set((items || []).map(v => v?.user_id).filter(id => id != null)));
      ...
      <div className="bg-white text-black rounded mt-2">
        <div className="h-[500px] overflow-y-auto p-4">
          {loading ? (
            ...
            <p>{t('visit.noMarketingTasks') || 'Brak zadań marketingowych.'}</p>
          ) : (
            <ul className="space-y-2">
              {items.map((v) => {
                const dateStr = v?.visit_date || '—';
                const author = userLabels[String(v?.user_id)] || '—';
                const text = (v?.marketing_tasks ?? '').trim();
                return (
